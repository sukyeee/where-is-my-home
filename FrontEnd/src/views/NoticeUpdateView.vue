  ...
  <div class="notice">
    ...
    <div class="view-content">
      ...
      <div class="container d-flex flex-wrap flex-column">
        ...
        <base-button @click="noticeUpdate" class="mt-3 w-25 form-group mx-auto">수정</base-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    initUI() {
      this.title = this.$store.state.notices.title;
      this.CKEditor.setData(this.$store.state.notices.content);
      this.category = this.$store.state.notices.category;
    ...
    async noticeUpdate() {
      ...
      try {
        let noticeId = this.$store.state.notices.noticeId;
        let { data } = await http.post("/notices/" + noticeId, formData, options);

        if (data.result == "login") {
          this.doLogout();
        } else {
          alertify.success("글이 수정되었습니다.", 1.5);
          this.$router.push({ name: "notice" });
        }
      } catch (error) {
        console.log(error);
        alertify.error("수정을 실패하였습니다.", 1.5);
      }
    },
    doLogout() {
