...
<div>   
            ...
            <div v-if=$store.state.isLogin class="d-flex align-items-center py-3" >
                <img class="rounded-circle mr-3" src="https://picsum.photos/250/100?1" alt="" style="width: 40px; height: 40px;">
            ...
            <div v-for="(comment, index) in listGetters" :key="index" >
                <div class="py-1 border-bottom mt-3"  v-if="comment.parentCommentId == 0"> 
                    <single-comments :comment="comment" :qnaId="comment.qnaId"  :commentId="comment.commentId" ></single-comments>
                    <reply-comments :qnaId="comment.qnaId" :parentCommentId="comment.commentId" :commentLists=listGetters ></reply-comments>
                </div>
            </div>
    </div>
